...
import { RESPONSE_PASSTHROUGH_METADATA, ROUTE_ARGS_METADATA } from "@nestjs/common/constants";
import { RouteParamtypes } from "@nestjs/common/enums/route-paramtypes.enum";
import { Reflector } from "@nestjs/core";
import { ORIGINAL_RENDER_FN, RenderFn, renderWithLayout } from "express-ejs-layouts";
...
export class AsyncRenderInterceptor implements NestInterceptor {
  ...
  private isInLibraryMode(context: ExecutionContext) {
    ...
    const hasPassthroughEnabled =
      Reflect.getMetadata(RESPONSE_PASSTHROUGH_METADATA, controller, methodName) === true;

    const handlerArguments = Reflect.getMetadata(ROUTE_ARGS_METADATA, controller, methodName) ?? {};
    ...
    const hasManualResponse = Object.keys(handlerArguments).some((key) => {
      ...
      return parameterType === RouteParamtypes.RESPONSE || parameterType === RouteParamtypes.NEXT;
    });
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    const view = this.reflector.get<string>(asyncRender.metadataKey, context.getHandler());
