# oidc-provider Adapters
Adapters are instantiated for each model from oidc-provider. The library
uses them to manage the storage of these models. The following documentation
of each required method of an adapter is a copy from library's
[`MyAdapter` example implementation]. It may be outdated.

[`MyAdapter` example implementation]: https://github.com/panva/node-oidc-provider/blob/main/example/my_adapter.js

## constructor
Creates an instance of MyAdapter for an oidc-provider model.
- name: Name of the oidc-provider model. One of "Grant, "Session", "AccessToken",
...
- id: Identifier that oidc-provider will use to reference this model instance for
    future operations.
- payload: Object with all properties intended for storage.
- expiresIn: Number of seconds intended for this model to be stored.
...
- id: Identifier of oidc-provider model
...
- userCode: the user_code value associated with a DeviceCode instance

## findByUid
Return previously stored instance of Session by its uid reference property.
- uid: the uid value associated with a Session instance
...
- grantId: the grantId value associated with a this model's instance
