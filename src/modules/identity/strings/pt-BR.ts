...
ptBR["identity:person:cpf:non-numeric-chars"] = (args, identifier) => {
  if (!args.argumentName)
    console.warn(
      `The message string '${identifier}' expected ` + `an \`args.argumentName\` property.`,
    );
...
ptBR["identity:users-repository:find-user-by-username:resource-not-found"] = (
  ...
  if (typeof username === "undefined") {
    console.warn(`The message string '${identifier}' expected an \`args.username\` property.`);
...
ptBR["identity:people-repository:find-by-id:resource-not-found"] = ({ personId }, identifier) => {
  if (!personId) {
    console.warn(`The message string ${identifier} expected an 'args.personId' property.`);
